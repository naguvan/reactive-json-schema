import { IObjectConfig, IObjectMetaProps } from "../Object";
...
test("test metafy with valuefy", () => {
  ...
  const type = valuefy(metafy(config, meta), value) as IObjectConfig;

  expect(type.properties!.agree.meta!.value).toBe(true);
  expect(type.properties!.color.meta!.component).toBe("color");
  expect(type.properties!.color.meta!.value).toBe("#ffffff");
  expect(type.properties!.size.meta!.component).toBe("range");
